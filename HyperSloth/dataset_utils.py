    ...
    **kwargs,
    ...
    if os.path.exists(dataset_name_or_path):
        ...
        try:
            ...
            loaded_dataset = load_dataset(dataset_name_or_path, split=split)

            # Handle different dataset types
            if hasattr(loaded_dataset, "__getitem__"):
            ...
            if isinstance(loaded_dataset, Dataset):
                ...
                # Handle DatasetDict or other types
                if hasattr(loaded_dataset, "values"):
                    # DatasetDict case - take first split
                    dataset = list(loaded_dataset.values())[0]
                else:
                    raise ValueError(
                        f"Unsupported dataset type: {type(loaded_dataset)}"
                    )
